...
export default function PatternSequence() {
  ...
  const loadQuestion = async () => {
    setLoading(true);
    setSelected(null);
    const q = await clientAIGenerator.generateQuestion({ gameType: 'pattern', subject: 'Logic' });
    setData(q);
    setLoading(false);
  };
  ...
  if (loading) {
    return (
      <div className="bg-card rounded-3xl p-8 text-center ring-1 ring-border/50">
        ...
        <p className="text-sm font-bold text-muted-foreground">Generating pattern logic challenge...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card rounded-3xl p-6 sm:p-8 shadow-soft ring-1 ring-border/50 max-w-2xl mx-auto text-center">
      <div className="flex items-center justify-between mb-6">
        <span className="text-xs font-bold uppercase tracking-wider bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full">
          🔢 Pattern Sequence ({data.generatedBy})
        </span>
        <span className="text-sm font-extrabold text-foreground">Score: {score} XP</span>
      </div>

      <h3 className="text-xl font-extrabold text-foreground mb-4">{data.question}</h3>
        ...
        {data.options.map((opt, i) => {
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl border-2 text-2xl font-black transition-all flex items-center justify-center gap-2 min-h-[60px] ${style}`}
            >
              <span>{opt}</span>
            </button>
          );
      ...
      {selected !== null && (
        ...
        >
          <RefreshCw className="w-4 h-4" /> Next Pattern
        </button>
      )}
    </div>
  );
